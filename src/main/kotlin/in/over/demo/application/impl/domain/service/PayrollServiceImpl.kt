...
class PayrollServiceImpl(
    ...
    override fun createForInterval(
        ...
        require(intervalStart < intervalEnd) { "intervalStart must be before intervalEnd" }
        require(intervalStart.nano == 0 && intervalEnd.nano == 0) { "payroll intervals use epoch-second precision" }
        validateHourlyRate(hourlyRate)
        if (employeeRepository.findByIdForUpdate(employeeId) == null) return null
        require(
            !repository.existsByEmployeeIdAndDeletedAtIsNullAndIntervalStartLessThanAndIntervalEndGreaterThan(
                employeeId,
                intervalEnd,
                intervalStart,
            ),
        ) { "payroll interval overlaps an active record" }
    ...
    override fun softDeleteStale(employeeId: Long, staleBefore: Instant): Int? {
        if (employeeRepository.findByIdForUpdate(employeeId) == null) return null
        return repository.softDeleteStale(employeeId, staleBefore, Instant.now())
    }
    ...
    private fun workedSeconds(payroll: PayrollRecord): Long {
        ...
        return if (mergedStart == null) total else Math.addExact(total, mergedEnd - mergedStart)
    }
